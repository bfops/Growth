...
-- | Create a Behaviour which, upon termination, will first check some other Behaviour for confirmation
       ...
       -> Behaviour
b `except` ex = Stream $ \seeds -> (`except` ex) <$$> (b $< seeds)
                               <|> case ex $< seeds of
                                    Nothing -> Just ((), b `except` ex)
                                    _ -> Nothing
...
transformations Rock =
        [ count 32 water `except` rockToLava =>> Dirt
        , volcano
        , rockToLava `except` lavaToRock =>> Lava False
        , magmify
        ]
    where
        rockToLava = heat 4
